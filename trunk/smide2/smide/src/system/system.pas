...
var
  Dest: WideChar;
begin
  Result := MultiByteToWideChar(DefaultUserCodePage, MB_ERR_INVALID_CHARS, Src, SrcBytes, @Dest, 1) <> 0;
{$ENDIF}
{$IFDEF LINUX}
begin
...
asm
    // Linux:
    // Setting ExitCode to non-zero in library initialization to force
    // module load failure is not supported in Linux.  There is no way to
    // communicate failure back to the Linux loader.
    // Upon error in library initialization, all initialized units will be
    // finalized.  Subsequent calls into exported library functions that
    // make use of initialized data or RTL routines will fail.
    // ExitCode <> zero indicates that the library failed its initialization.
...
@@append:
        ...
        MOV     EAX,[ESP]        // ptr to str into EAX
        MOV     ESI,[EDI-skew].StrRec.Length  // save old size before realloc
        CALL    _LStrSetLength
        MOV     EDI,[ESP]        // append other strs to dest
        PUSH    [EDI]
...
function _setlocale(__category: Integer; __locale: PChar): PChar; cdecl;
external libc name 'setlocale';
{$ENDIF}
